...
class Search extends React.Component{
    ...
    handleClick(){ 
        if (this.state.search == null) return null;
            ...
            let keywordsList=this.state.search.split(' ');
            let searchKeywordsList=[];
            let wordsSkipped=[];
            keywordsList.forEach(word => {
                if (!skipWordsList.includes(word)) searchKeywordsList.push(word);  
                else wordsSkipped.push(word);          
            });
            let displayIdsForAllTerms=[];
            ...
            if (this.props.history.location.pathname==='/searchResults'){
                this.props.history.replace({
                    ...
                    resultsForAllTerms: displayIdsForAllTerms,
                    resultsForSomeTerms: displayIdsForSomeTerms,
                    skippedWords: wordsSkipped,
                    valueSearched: this.state.search
                })
            }
            else{
    ...
    redirectToOption = (selectedOptionValue) => {
        this.setState({openMenu:false});
